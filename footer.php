...
<!-- Mirrored from demo.dashboardpack.com/cryptocurrency-html/ by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 11 Apr 2021 12:33:48 GMT -->
</html>

<script type="text/javascript">
$(document).ready(function(){
    setInterval(() => {user_status().fadeIn();}, 1000);
    setInterval(() => {admin_quote_status().fadeIn();}, 1000);
    setInterval(() => {contact_status().fadeIn();}, 1000);
    // setInterval(() => {admin_quote_status().fadeIn();}, 1000);
...
// admin quote status
function admin_quote_status(){
 $.ajax({
 url:"quote_status.php",
 type:"POST",
 success: function(data){
     if(data > 0){
         $("#quote_status").text(data);
        }else{
            $("#quote_status").remove();;
        }
...
// change admin quote status

$("#quote_class").click(function (e) { 
   $.ajax({
       type: "method",
       url: "change_quote_status.php",
       success: function (response) {
               $("#quote_status").fadeOut();
       }
   });
...
function contact_status(){
 $.ajax({
 url:"contact_us_status.php",
 type:"POST",
 success: function(data){
     if(data > 0){
         $("#contact_status").text(data);
        }else{
            $("#contact_status").remove();;
        }
...
// change Contact-Us status

$("#contact_class").click(function (e) { 
   $.ajax({
       type: "method",
       url: "change_contact_status.php",
       success: function (response) {
